...
(s/defmethod database.customer/insert! :sqlite :- models.customer/Customer
  [customer :- models.customer/Customer
   database-conn]
  (jdbc/with-transaction [tx database-conn]
    (-> (jdbc/execute! tx
...
(s/defmethod database.customer/by-username :sqlite :- (s/maybe models.customer/Customer)
  [username :- s/Str
   database-conn]
  (let [customer (-> (jdbc/execute! database-conn ["SELECT * FROM customers WHERE username = $1" username]) first)]
    (when customer
      (->> (roles-by-customer-id (UUID/fromString (:customers/id customer)) database-conn)
           (adapters.customer/sqlite->internal customer)))))

(s/defmethod database.customer/lookup :sqlite :- (s/maybe models.customer/Customer)
  [customer-id :- s/Uuid
   database-conn]
  (let [customer (-> (jdbc/execute! database-conn ["SELECT * FROM customers WHERE id = $1" (str customer-id)]) first)
        roles (roles-by-customer-id (UUID/fromString (:customers/id customer)) database-conn)]
...
(s/defmethod database.customer/add-role! :sqlite :- (s/maybe models.customer/Customer)
  [customer-id :- s/Uuid
   role :- s/Keyword
   database-conn]
  (insert-role! role customer-id database-conn)
  (database.customer/lookup customer-id database-conn))
